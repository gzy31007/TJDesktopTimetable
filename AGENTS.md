...
- **缩放（2026-09-15 二次定稿：原生循环 → 自实现）**：
  ...
  - **已知取舍（用户已验收通过，不必再改）**：顶部光标条（6px，`HorizontalAlignment=Stretch`）
    压在顶部条上，会吃掉那 6px 的 pointerdown —— 即"贴着窗口最上沿那一条按下去拖窗口"可能不响应。
    这是 DeskBox 也有的同类折中（它的 resize 边框同样盖在标题栏上）。
  - **验收状态（2026-09-15 三次定稿，全绿）**：自实现缩放（判定 + 跟手 + 落盘）与
    **渲染层缩放光标**均已真机确认。回归时按下面三步走，别一上来改代码 ——
    ① `.tools/verify-resize.ps1`（几何/落盘/恢复）；② 真机看边缘有没有缩放光标；
