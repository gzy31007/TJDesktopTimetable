...
- **新版本提示**（DeskBox 同款机制，只做到"提示 + 打开下载页"）：
  - 分层：纯逻辑 `TjtCore/UpdateCheck.cs`（单测 `UpdateCheckTests.cs`）—— tag 解析（允许 `v` 前缀 / 预发布后缀，比较只看前三段）、只认正式 Release（`prerelease` / `draft` 一律不提示）、跳过语义（`Skipped` 只压住被跳过的那个版本）、资产匹配（`-win-x64.zip`）、`DownloadUrlFor`（没匹配到资产就退回 Release 页）、**镜像拼接**（`ProxyPrefix` / `CanProxy` / `ProxiedUrl` / `WithProxyPrefix`）；网络在 `Tjt.App/Data/UpdateChecker.cs`（UA `TJDesktopTimetable/<版本>`、**失败一律静默**只记 `[update] …`）。
  - **gh-proxy.org 兜底**（2026-09-17 用户要求）：查询先直连 `api.github.com`（超时 **6s**，被墙时秒级失败），直连拿不到响应就用 `https://gh-proxy.org/<原地址>` 重试一轮（超时 20s，日志 `[update] 直连未拿到响应，改用镜像兜底：…`，结论那行带 `来源 直连/镜像`）。**`DownloadUrlFor` 返回的下载链接本身就是镜像地址**（国内点开就能下）。
...
- **入口与设置界面**：
  - **顶部条**（`Rendering/BoardRenderer.cs`）：左图标 + 学期 + 「第 N 周 · 今日 N 节」；右 **刷新** + **⋯ 图标**（`Rendering/IconGlyph.cs`，Segoe Fluent Icons）。`⋯` 菜单：设置 / 导入课表… / 重新载入课表 / 恢复默认位置 / 贴桌面层勾选 / 显示周末勾选 / **周次视图子菜单（四选一）** / 隐藏 / 退出。动作经 `WidgetActions`（渲染层只发意图，逻辑在 `MainWindow.BuildActions()`）。
  - **托盘**（`Win32/TrayIcon.cs`）：`Shell_NotifyIcon` + `HWND_MESSAGE` 专用窗口 + `TrackPopupMenu(TPM_RETURNCMD)` 原生菜单（同步取回选中项）。左键 = 显示挂件；菜单 = 显示 / 设置 / 导入课表… / 重新载入 / 恢复位置 / 贴桌面层勾选 / 显示周末勾选 / **周次视图子菜单（普通勾）** / 退出。图标 `Assets/app.ico`（运行时 `LoadImage`）。WASDK 1.8 没有托盘 API，直接 P/Invoke。
...
- **外壳能力**：默认工作区右下角（留 24px，用 `DisplayArea.WorkArea` 免得压任务栏）；顶部「学期 · 第 N 周 · 今日 N 节」；尺寸变化即重排。
  ...
  - `--size WxH` 真改窗口尺寸；其余开关（`--fixture` / `--log` / `--no-backdrop` / `--desktop-layer` / `--weekend` / `--week-view` / `--today` / `--now HH:mm` / `--material`）见 `AppStartupOptions`。
...
- **周次视图**（2026-09-21 定稿，见 `CONTEXT.md` 与 `docs/adr/0001`/`0002`）：`Weeks.WeekView { All, Current, Odd, Even }`（枚举由 `WeekFilter` 改名），`BoardOptions.WeekView` **默认 `Current`（只看本周）** —— 写成 `All` = 功能白做。**兜底**：`CurrentWeek == null`（开学前 / 学期结束后 / 开学日未知）→ `Weeks.ResolveFilter` 返回 `null` = 不过滤，静默退回显示全部周次（挂件绝不空，header 照旧「假期」）。视图只决定画哪些：**冲突判定 / 今日高亮 / 「今日 N 节」一概不受它影响**；被本周丢掉的时段计入 `HiddenSessions`（与单双周同口径），「隐藏周末」那条例外不变。
  - **「今日 N 节」口径已修正**：`BoardState.TodaySessionCount`（= 今天 × **当前教学周**命中的安排数；假期退回"今天的全部安排"；空掩码不算），`Tjt.Widget/BuildHeader` 读它，**不再**数 `state.Blocks`（旧实现会把别的周的课算进今天，`All` 视图下数字也变了）。
  - **`--today YYYY-MM-DD` 是"今日"的唯一真源**（今日高亮 / 当前周 / 今日节数），`--week-view all|current|odd|even` 只影响本次运行、不落盘。⚠️ **凡断言观感的验收/冒烟必须显式带 `--today`**（`build-winui.ps1 -RunSmoke` 默认 `--today 2026-09-21` = 第 2 周，基准 `blocks=14 hidden=5 todaySessions=2`；`--week-view all` 才是旧的 `blocks=19`）—— 漏了就随日历漂，一周后周期性假 FAIL。日志锚点 `[weekview] view=… today=… week=… todaySessions=… hidden=… menu=4`；验收 `.tools/verify-weekview.ps1`（`verify-weekend.ps1` / `verify-sjtu.ps1` 已显式钉 `--week-view all --today 2026-09-21`）。
  - **三入口同源**读 `MainWindow.WeekViewEnabled`（`_options.WeekView ?? _settings.WeekView`）：设置「外观」页 `ComboBox`、挂件 `⋯` 菜单「周次视图」子菜单（`RadioMenuFlyoutItem`）、托盘同名子菜单（**普通勾 `MF_CHECKED`**，别试 `MFT_RADIOCHECK` —— 不设菜单位图时它不画圆点）。托盘子菜单靠 `TrayMenuItem.Children` + `MF_POPUP`（`AppendMenuW` 第三参是 `UINT_PTR`，P/Invoke 必须是 `nint`；子菜单只让父菜单销毁，别再 `DestroyMenu` 一次）。文案唯一真源 = `Tjt.Widget/WeekViewLabels`（两端同字）。Linux 线只有 `⋯` 菜单，别在那边"补齐"三入口。
  - 落盘：`WidgetSettings.WeekView` 挂 `[JsonConverter(typeof(JsonStringEnumConverter))]` 写成字符串（`"WeekView": "Current"`；**不动全局 `Options`**，`ThemeMode`/`MaterialMode` 仍是数字；未知字符串 → 整份回落默认。注：该转换器默认 `allowIntegerValues=true`，手写数字会按枚举位置解释）。旧布局测试凡不测视图的都显式 `WeekView = WeekView.All`，别让"当前周"搅动几何/并排断言。
...
- **课表导入**：入口三处（设置「导入」页、托盘「导入课表…」、挂件 `⋯` 菜单「导入课表…」）都落到同一页；功能对齐 Electron 侧导入面板（粘请求抓取 / 粘或选 JSON / 适配器探测 / 探测行与诊断 / 清空 / 重新载入 / 打开数据目录）。
  ...
  - 载入顺序（`AppHost.Load`，唯一入口，启动与"重新载入"共用）：`--fixture` 显式指定 → 用户导入的 `timetable.json` → **内置示例课表 `DemoData`**（虚构数据）；`LoadedTimetable.Origin` 记来源（设置页文案 + 日志读它）。
    ⚠️ **没有"回退到 `dotnet/fixtures/`"这一步**（2026-09-17 改）：那里面是脱敏后的**真实抓包**，只服务于测试与 `--fixture <path>`；早先拿它当默认课表，用户会看到一张真实课表并以为"程序怎么有我的数据"。`Tjt.App` / `Tjt.Linux` 的 csproj 因此也**不再**把 fixtures 复制进输出/发布目录（发布包里没有任何真实课表数据）。
  ...
  - "清空课表"删的就是 `timetable.json`（之后按载入顺序回退到**内置示例课表**），对话框如实写。
...
- **冒烟组合**：`-RunSmoke`（`[backdrop] mode=mica-controller`）、`-RunSmoke -NoBackdrop`（`[backdrop] skipped`）；两者都产出 `[smoke] ok blocks=14 ...`（`-RunSmoke` 已默认带 `--today 2026-09-21` + 默认「只看本周」；想要旧的 19 块用 `-WeekView all`），另有 `[weekview] view=current …`；`-DesktopLayer` 另验 `[desktop-layer] attach=ok owner=0x...` 与 `send-to-bottom=ok`。
